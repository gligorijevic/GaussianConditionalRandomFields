function [y S R x y_unc S_unc R_unc x_unc alpha beta theta] = temporalGraph5(T,N,alpha,beta,sparseness)
%     clear;
%     clc;
%     N = 5;
%     T = 5;
...
%     beta = 0.1;
    ...
    %% GENERATE R
    no_features = 100;
    x = random('uniform', 25, 30, N, no_features, T); % uniform(0,1) -> Matrica (1(features),N,T)
    
%     theta = ones(no_features, N);  % separate theta for each node!
    theta = random('uniform', -1,1,no_features,1);
...
%     theta = rand(no_features,1);
    ...
    x_unc = [reshape(ipermute(x(:,:,1:T-1),[1 3 2]),[],100); x_unc];
